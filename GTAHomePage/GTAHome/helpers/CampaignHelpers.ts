...
export class CampaignHelpers {
    private static xrm: any = (window.parent as any).Xrm || (window as any).Xrm;
    ...
    static async getPatrolCampaigns(
        ...
        return await this.getCampaignsByOrgUnit(orgUnitId, {
            status: status, // 1 = Not Started, 2 = In Progress, 100000004 = Completed
            campaignType: 100000004, // Patrol type (if different)
    ...
    static async isNaturalReserveCampaign(campaignId: string): Promise<boolean> {
        try {
            const campaign = await this.xrm.WebApi.retrieveRecord(
                'new_inspectioncampaign',
                campaignId,
                '?$select=_duc_organizationalunitid_value&$expand=duc_organizationalunitid($select=duc_englishname)'
            );
