    ...
    while ((Date.now() - start) < maxWaitMs) {
        const result = await Xrm.WebApi.retrieveMultipleRecords(
            ...
            `?$select=msdyn_workorderservicetaskid
             &$filter=msdyn_workorder eq ${workOrderId}
...
async function uLA() {
    try {
        ...
        var stageActions = await Xrm.WebApi.retrieveMultipleRecords(
            "duc_stageaction",
            `?$select=duc_stageactionid,duc_defaultstatus
              &$filter=duc_canbetriggeredbytarget eq true
              and duc_process eq ${processDefinitionId}
              and duc_relatedstage eq ${currentStageId}
              &$top=10`
        );
        ...
        for (let item of stageActions.entities) {
            let statusId = item.duc_defaultstatus || item._duc_defaultstatus_value;
    ...
    try {
        ...
        var bookingResult = await Xrm.WebApi.retrieveMultipleRecords(
            ...
            `?$select=bookableresourcebookingid
              &$filter=msdyn_workorder eq ${woId}
              &$orderby=createdon asc
              &$top=1`
        );
...
async function createDailyInspection() {
    try {
        ...
        const attendanceRes = await Xrm.WebApi.retrieveMultipleRecords(
            "duc_attendance",
            `?$select=duc_attendanceid&$filter=duc_user eq ${createdBy} and createdon ge ${todayStart.toISOString()} and createdon lt ${nextDay.toISOString()}`
        );
...
async function hasWorkOrderServiceTask(workOrderId) {
    ...
    const result = await Xrm.WebApi.retrieveMultipleRecords(
        "msdyn_workorderservicetask",
        `?$select=msdyn_workorderservicetaskid&$filter=msdyn_workorder eq ${cleanWorkOrderId}&$top=1`
    );
...
function WO_CheckAllowedDistance() {
    ...
    function checkNearestAccountAddress(allowedDistance) {
        ...
        options += "&$filter=duc_account eq " + accountId;
