...
async function uLA_FromBooking(workOrderId) {
    ...
    try {
        ...
        await runOfflineActionLogic(actionIdToSet, peId, workOrderId);
...
// Self-contained offline plugin logic for start_inspection_bookable.js
async function runOfflineActionLogic(actionId, processExtensionId, workOrderId) {
    ...
    try {
        ...
        if (targetStatusField && statusValue) {
            ...
            else if (existingStatusEntity) { woUpdate[targetStatusField + "@odata.bind"] = "/" + _entitySetNameBookable(existingStatusEntity) + "(" + statusValue + ")"; }
            hasWoUpdate = true;
        }
        if (targetSubStatusField && subStatusValue) {
            ...
            else if (existingSubStatusEntity) { woUpdate[targetSubStatusField + "@odata.bind"] = "/" + _entitySetNameBookable(existingSubStatusEntity) + "(" + subStatusValue + ")"; }
            hasWoUpdate = true;
        }
...
function _entitySetNameBookable(logicalName) {
    var map = {
