...
function WO_OpenDirectionsToDucAddress() {
    ...
    var MSG = (lang === 1025) ? { 
        ...
        noAddresses: "لم يتم العثور على عناوين مرتبطة بالحساب الفرعي أو أمر العمل.",
        noCoords: "بعض العناوين لا تحتوي على إحداثيات صالحة.",
        cannotGetLoc: "تعذر استرداد الموقع الحالي، يرجى مراجعة إعدادات الموقع في الهاتف.",
        confirmTitle: "الاتجاهات في خرائط جوجل",
        confirmText: "هل ترغب في فتح خرائط جوجل للحصول على الاتجاهات مع نقاط التوقف المتعددة؟",
        tooManyWaypoints: "تم العثور على أكثر من 25 عنوانًا. سيتم استخدام أقرب 25 موقعًا فقط.",
        loadingMessage: "جاري تحميل العناوين وبناء المسار...",
        noAddressLookup: "لم يتم العثور على عنوان في حقل العنوان",
        usingWorkOrderAddress: "لم يتم العثور على عناوين متعددة. استخدام العنوان من أمر العمل."
    } : { 
        noSubAccount: "No sub-account is selected on the Work Order ",
        noAddresses: "No addresses found linked to the sub-account or work order.",
        noCoords: "Some addresses do not have valid coordinates.",
        cannotGetLoc: "Can't get current location. Please check your mobile location settings.",
        confirmTitle: "Google Maps Directions",
        confirmText: "Open Google Maps to get directions with multiple stops?",
        tooManyWaypoints: "Found more than 25 addresses. Only the nearest 25 locations will be used.",
        loadingMessage: "Loading addresses and building route...",
        noAddressLookup: "No address found in the address field .",
        usingWorkOrderAddress: "No multiple addresses found. Using address from work order."
    };
    ...
    function processAddresses(addresses, originLat, originLng) {
        ...
        return processedAddresses.length > 0 ? processedAddresses : null;
    }

    function openGoogleMaps(addresses, originLat, originLng) {
        var maxWaypoints = 25;
        if (addresses.length > maxWaypoints) {
            addresses = addresses.slice(0, maxWaypoints);
            Xrm.Navigation.openAlertDialog({ text: MSG.tooManyWaypoints });
    ...
    function tryGetAddressFromLookup(workOrderId, originLat, originLng) {
        // Try to get the address from the duc_address lookup field on the work order
        var addressAttr = Xrm.Page.getAttribute("duc_address");
        var addressVal = addressAttr ? addressAttr.getValue() : null;
        
        if (!addressVal || !addressVal[0] || !addressVal[0].id) {
            Xrm.Utility.closeProgressIndicator();
            Xrm.Navigation.openAlertDialog({ text: MSG.noAddressLookup });
            return;
        }

        var addressId = addressVal[0].id.replace(/[{}]/g, "");

        // Retrieve the address information record
        Xrm.WebApi.retrieveRecord("duc_addressinformation", addressId, "?$select=duc_latitude,duc_longitude").then(
            function (addressRecord) {
                if (!addressRecord.duc_latitude || !addressRecord.duc_longitude) {
                    Xrm.Utility.closeProgressIndicator();
                    Xrm.Navigation.openAlertDialog({ text: MSG.noCoords });
                    return;
                }
                ...
                var singleAddress = [{
                    lat: addressRecord.duc_latitude,
                    lng: addressRecord.duc_longitude,
                    distance: calculateDistance(originLat, originLng, addressRecord.duc_latitude, addressRecord.duc_longitude)
                }];

                openGoogleMaps(singleAddress, originLat, originLng);
            },
            function (error) {
                Xrm.Utility.closeProgressIndicator();
                Xrm.Navigation.openAlertDialog({
                    text: (error && error.message) ? error.message : "Failed to retrieve address from lookup field."
                });
            }
        );
    }

    function tryGetAddressesByWorkOrder(workOrderId, originLat, originLng) {
        // Priority 2: Get addresses related to the work order
        var options = "?$select=duc_addressinformationid,duc_latitude,duc_longitude";
        options += "&$filter=_duc_msdyn_workorder_value eq " + workOrderId;
        options += " and duc_latitude ne null and duc_longitude ne null";

        Xrm.WebApi.retrieveMultipleRecords("duc_addressinformation", options).then(
            function (result) {
                var addresses = processAddresses(result.entities, originLat, originLng);
                
                if (addresses && addresses.length > 0) {
                    openGoogleMaps(addresses, originLat, originLng);
                } else {
                    // Priority 3: Try to get single address from lookup field
                    tryGetAddressFromLookup(workOrderId, originLat, originLng);
                }
            },
            function (error) {
                Xrm.Utility.closeProgressIndicator();
                Xrm.Navigation.openAlertDialog({
                    text: (error && error.message) ? error.message : "Failed to retrieve address records by work order."
                });
            }
        );
    }

    function tryGetAddressesBySubAccount(accountId, workOrderId, originLat, originLng) {
        // Priority 1: Get addresses related to the sub-account
        var options = "?$select=duc_addressinformationid,duc_latitude,duc_longitude";
        options += "&$filter=_duc_account_value eq " + accountId;
        options += " and duc_latitude ne null and duc_longitude ne null";
        ...
        Xrm.WebApi.retrieveMultipleRecords("duc_addressinformation", options).then(
            function (result) {
                ...
                if (addresses && addresses.length > 0) {
                    ...
                    // Priority 2: If no addresses found by sub-account, try by work order
                    tryGetAddressesByWorkOrder(workOrderId, originLat, originLng);
                }
            },
            function (error) {
                Xrm.Utility.closeProgressIndicator();
                Xrm.Navigation.openAlertDialog({
                    text: (error && error.message) ? error.message : "Failed to retrieve address records by sub-account."
                });
            }
        );
    }
    ...
    Xrm.Device.getCurrentPosition().then(function (location) {
        ...
        // Get the work order ID for the current record
        var workOrderId = Xrm.Page.data.entity.getId().replace(/[{}]/g, "");

        // Check if sub-account is selected
        var subAccountAttr = Xrm.Page.getAttribute("duc_subaccount");
        var subAccountVal = subAccountAttr ? subAccountAttr.getValue() : null;
        
        if (subAccountVal && subAccountVal[0] && subAccountVal[0].id) {
            // Priority 1: Try to get addresses by sub-account
            var accountId = subAccountVal[0].id.replace(/[{}]/g, "");
            tryGetAddressesBySubAccount(accountId, workOrderId, originLat, originLng);
        } else {
            // No sub-account, skip to Priority 2: Try by work order
            tryGetAddressesByWorkOrder(workOrderId, originLat, originLng);
        }
