...
async function oFST() {
    try {
        if (!Xrm || !Xrm.Page || !Xrm.Page.data || !Xrm.Page.data.entity) {
            console.error("Error #3: Form context not available in oFST()");
            Xrm.Navigation.openAlertDialog({ text: "Error #3: Form context not available in oFST()" });
            return;
        }
...
async function uLA() {
    ...
    try {
        if (!Xrm || !Xrm.Page || !Xrm.Page.data || !Xrm.Page.data.entity) {
            console.error("Error #4: Form context not available in uLA()");
            Xrm.Navigation.openAlertDialog({ text: "Error #4: Form context not available in uLA()" });
            return;
        }

        var woId = Xrm.Page.data.entity.getId().replace(/[{}]/g, "");
        var form2 = Xrm.Page;
        var workorder5 = form2.getAttribute("msdyn_workorder");
        if (workorder5 != null) {
            woId = workorder5.getValue();
        }
        else {
            woId = Xrm.Page.data.entity.getId().replace(/[{}]/g, "");
        }
...
async function sBIP() {
    try {
        if (!Xrm || !Xrm.Page || !Xrm.Page.data || !Xrm.Page.data.entity) {
            console.error("Error #5: Form context not available in sBIP()");
            Xrm.Navigation.openAlertDialog({ text: "Error #5: Form context not available in sBIP()" });
            return false;
        }
...
function IsFromMobile() {
    if (!Xrm || !Xrm.Page || !Xrm.Page.context || !Xrm.Page.context.client) {
        console.error("Error #2: Xrm.Page context not available in IsFromMobile()");
        return false;
    }
...
async function createDailyInspection() {
    try {
        ...
        if (!Xrm || !Xrm.Page || !Xrm.Page.data || !Xrm.Page.data.entity) {
            console.error("Error #6: Form context not available in createDailyInspection()");
            Xrm.Navigation.openAlertDialog({
                text: "Error #6: Form context not available in createDailyInspection()"
            });
            Xrm.Utility.closeProgressIndicator();
            return;
        }
...
function WO_CheckAllowedDistance() {
    if (!Xrm || !Xrm.Page || !Xrm.Page.data || !Xrm.Page.data.entity) {
        console.error("Error #7: Form context not available in WO_CheckAllowedDistance()");
        Xrm.Navigation.openAlertDialog({ text: "Error #7: Form context not available in WO_CheckAllowedDistance()" });
...
async function setWorkOrderFieldsFromAccount_WO() {
    try {
        ...
        console.error("setWorkOrderFieldsFromAccount_WO error:", error);
    }
}
...
async function checkInspectorPermission() {
    try {
        ...
        console.error("checkInspectorPermission error:", e);
        return false;
    }
}
